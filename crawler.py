#!/usr/bin/env python3
"""
関東船釣り情報クローラー v4.1 (fishing-v.jp版)
変更点: history.json連携 / 昨年同週比較 / copyright年修正
"""
    ...
    for table in tables:
        ...
        for row in table[1:]:
            ...
            results.append({
                ...
                "fish": fishes, "count_range": count, "size_range": size,
            })
    return results
...
# ============================================================
def load_history():
    """history.jsonを読み込む。なければ空dictを返す。"""
...
def get_week_key(year, week_num):
    return f"{year}/W{week_num:02d}"

def get_yoy_data(history, fish, year, week_num):
    """
    今週(year/Wnn) と 昨年同週(year-1/Wnn) の履歴データを返す。
    戻り値: (this_week_data, last_year_data) どちらも None の場合あり
    """
    this_key  = get_week_key(year,     week_num)
    last_key  = get_week_key(year - 1, week_num)
...
def yoy_badge(this_data, last_data):
    """
    昨年比バッジHTMLを返す。
    比較軸: avg (平均釣果) を優先。なければ ships (出船数)。
    """
    ...
    if not l_val or not t_val:
        return ""
    pct = round((t_val - l_val) / l_val * 100)
    if pct >= 0:
        return f'<span class="yoy up">↑昨年比+{pct}%</span>'
    else:
        return f'<span class="yoy down">↓昨年比{pct}%</span>'
...
def calc_targets(data, history):
    ...
    for fish, cnt in sorted(fish_counts.items(), key=lambda x: -x[1])[:10]:
        ...
        targets.append({
            "fish": fish, "count": cnt, "score": score,
            "comment": comment, "badge": badge,
        })
    return targets[:5]
...
.tc-fish{font-size:18px;font-weight:bold;color:#fff;margin-bottom:6px;display:flex;align-items:center;gap:8px;flex-wrap:wrap}
...
def build_html(catches, crawled_at, history):
    ...
    for fish, cs in sorted(fish_summary.items(), key=lambda x: -len(x[1])):
        ...
        for c in sorted(cs, key=lambda x: x["date"] or "", reverse=True)[:5]:
            cnt = (f"{c['count_range']['min']}〜{c['count_range']['max']}"
                   if c["count_range"] and c["count_range"]["min"] != c["count_range"]["max"]
                   else (str(c["count_range"]["min"]) if c["count_range"] else ""))
            sz = ""
            if c["size_range"]:
...
def build_fish_pages(data, history):
    ...
    for fish, catches in fish_summary.items():
        ...
        if this_w and last_w:
            ...
            def diff_cell(t_val, l_val, unit=""):
...
def build_calendar_page():
    ...
    header_cells = "".join(
        f'<th class="{"cur-month" if i+1==current_month else ""}">{m}</th>'
...
def main():
    ...
    print(f"=== 関東船釣りクローラー v4.1 (fishing-v.jp) 開始: {crawled_at} ===")
    ...
    with open("catches.json", "w", encoding="utf-8") as f:
        json.dump({"crawled_at": crawled_at, "total": len(all_catches),
                   "errors": errors, "data": all_catches},
