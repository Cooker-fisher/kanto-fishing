...
# 注意: 部分文字列の包含関係がある魚種は、具体的な名前を先に定義する
# 例: アマダイ→マダイ、キンメダイ→メダイ、アカムツ→ムツ、LTアジ→アジ
FISH_MAP = {
    # ── アジ系（LT/ビシを先に判定） ──
...
def guess_fish(t):
    ...
    具体的な名前（LTアジ等）がマッチしたら、汎用名（アジ等）を除外する。
    """
    ...
    to_remove = set()
    for m in matched:
        sup = _FISH_SUPPRESS.get(m)
        if sup and sup in matched:
            to_remove.add(sup)
    if to_remove:
        matched = [m for m in matched if m not in to_remove]
    return matched
...
SEASON_DATA = {
    ...
    "アカムツ": [4,4,3,3,3,3,3,3,3,3,4,4],  # 通年・冬がやや好調
