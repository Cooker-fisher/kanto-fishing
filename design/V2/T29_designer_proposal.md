...
<div class="ar">
  ...
  <span class="ar-trips">1便</span>
  <span class="ar-ships"><a href="../ship/miki-maru-tsuribuneten.html">三喜丸釣船店</a></span>
  <span class="ar-fa"><a href="../fish_area/aji-koshiba.html">詳細 →</a></span>
</div>
```
...
| 案 | テキスト | 文字数 | 評価 |
|---|---|---|---|
| A | `詳細 →` | 4字 | 汎用的・クリック率期待薄 |
| B | `小柴港のアジ詳細` | 8字 | SEO 的明示性高・長い |
| C | `詳細を見る` | 6字 | 自然・クリック率やや高 |

**推奨: 案 A「詳細 →」**

理由: area_cmp の各行はすでに情報密度が高い（エリア名・釣果レンジ・サイズ・便数・船宿）。追加のテキストは最小限にしてレイアウトを壊さないことを優先する。モバイルで `.ar` が折り返す際も `ar-fa` が最後尾に配置されれば収まりが良い。SEO 的には a タグのテキストより href の URL パターンと周辺コンテキストが効く。
...
### 判断 A: area_cmp のリンクテキスト

- 選択肢: 「詳細 →」（推奨・4字）vs 「{エリア名}の詳細」（8〜12字）
- 推奨理由: 列幅を圧迫しない。モバイルで折り返しの原因にならない。
- 懸念: 「詳細 →」はコンテキストが薄く、どこの詳細かが自明でない。
