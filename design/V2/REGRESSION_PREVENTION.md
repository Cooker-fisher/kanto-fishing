...
| 38 | `docs/fish/*.html`・`docs/fish_area/*.html`・`docs/ship/*.html`（T41 / 2026-06-07） | 「最高(実績は)N匹」「最大匹数 N匹」の N が西暦域 [1990,2035] または 1500 超でないこと。実在最大は数物でも 1000 未満（アジ713・スジイカ702）。西暦誤抽出（ヒラメ「2025匹」）・桁化けを弾く。ship ページも公開生成物なので対象（PR#51 レビュー指摘で追加）。crawler.py `_FISH_CNT_CAP` + ship 集計（`_ship_load_yearly_summary`/`_ship_load_seasonal_fish`）の `_is_plausible_cnt` 除外で担保 |
