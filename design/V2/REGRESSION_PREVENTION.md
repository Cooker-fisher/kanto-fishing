...
| 70 | `docs/area/*.html` thin テンプレの H1（2026-09-02） | **#68 の副作用で、当日釣果が無いページの H1 だけが【毎日更新】を名乗っていた**。thin テンプレ（当日釣果なし・20本）の title は 2026-08-06 の判断で `{エリア}（別称・県名）の船釣り釣果・過去実績` にしてあり、body も「本日の釣果報告は集計待ち」。ところが #68 で thin パスにも `area_h1_text()` を通した際、【毎日更新】が無条件で付いた。#68 の前提そのもの（**Google が title を捨てて H1 を SERP タイトルに使う**）が成り立つと、SERP で「毎日更新」と読ませてクリックさせた先が「集計待ち」になる。2026-08-06 の判断（嘘はつかない・ただし最初に見せる情報ではない）と正面から矛盾する。GSC 2026-07〜08 実績（**ページ次元＝真値**）で thin 20本は 975impr / 21click（2.15%）、full 134本は 14,636impr / 346click（2.36%）＝**ほぼ同じ**。⚠ 当初はクエリ次元 CSV で「thin 0.67% / full 2.28%」と見えていたが、これは GSC の匿名化欠測（thin ほど低頻度クエリ比率が高く落ちやすい）による幻だった。2026-08-06 に thin の title/desc を作り直した際の根拠「thin 0.35% / full 2.31%」も同じバイアスを受けている。**対策**: `area_h1_text(..., daily=False)` を追加し、thin パスの H1 末尾を 【毎日更新】ではなく「・過去実績」にして title と揃える（30文字上限・優先順位は #68 のまま）。**検証**: title に「・過去実績 |」を含むページの H1 に【毎日更新】が無いこと。negative test: `daily=False` を外す → 20本 fail。⚠ **CTR 改善の施策ではない**（thin の CTR は元から full 並み）。目的は「書いてあることを本当にする」ことで、AdSense 4連敗の理由（「有用性の低いコンテンツ」）に効くかは未証明。決定ログ「2026-09-02」参照 |
