...
| 66 | `crawler.py` `build_area_title_body()`（2026-08-09） | **area title の SERP 幅超過**。GSC 実測で 3〜6位帯の CTR が 2.05%（順位カーブの目安 8%）と下振れし、期待クリック比の不足ほぼ全額がこの帯だった。原因は title 154本中 **54本(35%)** が SERP 表示幅 600px を超過し、魚種名が途中で切れていたこと（勝浦「スル…メイカ」・江戸川放水路「シ…ーバス」）。**対策**: `／N船宿` は幅が余っている時だけ付ける（GSC 実クエリに船宿数を含むものは0件＝検索意図に無いのに常時5〜6字を消費していた）。超過 54→26本・魚種の欠落ゼロ。**魚種を削る最適化は保留**: モバイルの SERP は2行折り返しで desktop より広いが、`analytics/fetch_gsc.py` の DIMENSIONS が device を取っておらずモバイル比率が不明のため、「幅が足りない時に足さない」（どの device でも損しない）方向だけを実装した。**検証は docs ではなく生成関数**: docs は CI のフル実行でしか正しく再生成できず（2026-08-06 の `--area-only` の教訓）、docs を fail 条件にすると再生成前に CI が赤くなる。crawler.py を importlib で読み込み、現行 docs の title から復元した実入力69本で契約 (1) `／N船宿` が載るのは幅に収まる時だけ (2) 入力の魚種を1件も落とさない、を固定する。docs 側の超過本数は warn で可視化し、fail 化は保留中の #65（別称露出）と同時に CI 再生成後へ。決定ログ「2026-08-09」参照 |
