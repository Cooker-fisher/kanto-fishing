...
| 12 | `area/*.html` 海況セクション（2026-08-05 改訂） | 潮汐が名称（大潮/中潮等）・月相が名称（満月/新月等）・1行コメント有り。＋**エリア別予報の生存確認**: `area_coords.json` に座標があるページに限定し、`(水温, 波高, 風)` の種類が 座標数×0.6（最低9）以上。旧実装は「先頭40ファイルの水温が8種類以下なら warn」で、①先頭40件は座標なし＝設計上フォールバックするページが多数 ②台風8/3の攪拌で水温が実際に均質化 の2つで誤検知した（同 run は「46/46座標 OK」）|
...
| 56 | `docs/x_post/*.html` 最新1枚（2026-07-22） | 日次まとめ散文の**数値根拠**。**背景**: ハイライト／魚種別報告が `templates.py` の H/F 文型依存で「今期は好海況が観測されています」「再現性が高いと推察されます」「組み込みやすい魚種です」等の根拠なしフィラーが本文の大半だった（ユーザー指摘「薄っぺらい」）。裁定＝フィラー全廃・数値根拠のみ。`x_post/insights.py` が **data/V2 CSV のみ**（analysis.sqlite は .gitignore で CI 不在）から平年比（過去3年の同旬 cnt_max 中央値比・母数併記）・船宿別上位3便・記録性（N日ぶり／直近60日で最多）を算出し、`x_post/narrative.py` が散文とカード（最大4枚）を生成。翌日の予測引用は #53 と同じ関門（tier A かつレンジ有り）のみ。検証（最新1枚のみ・`class="commentary evidence"` を持つページに限る。旧ページは CSV 由来だと数値が変わるため遡及再生成しない）: (a) 禁止フィラー（`と推察されます`/`好海況が観測されています`/`再現性が高い`/`組み込みやすい魚種です`/`好スコア`/`予定通り運航しました`）が無い (b) 「平年比」の記述には母数「同じ旬（N便）」が併記される (c) 「最多は…」の船宿別記述がある。(b)(c) はフォールバック日を考慮し warn 止まり。決定ログ「2026-07-22」参照 **第2ラウンド（同日）**: 内部用語（tier A/同旬/母数/平年比/wMAPE/open_tier）の露出を fail、「例年」使用時は定義文（過去3年の同じ時期）を必須、ひらがな幼稚表現（きょういちばん/ふだんは/あすの予想）を fail、考察セクション『本日の条件と、これまでの傾向』があるなら相関注記「原因を特定したものではありません」を必須・因果断定表現を fail。考察は `insights.wx_baseline`（weather/*.csv から算出した過去3年同時期の海況目安）と `normalize/ship_analysis.json`（C層 combo_wx_params のドメインレビュー済み蒸留）の突き合わせで生成し、当日値と対応づく因子（sst/wave/wind/潮回り）に限定する。 |
| 57 | `normalize/fish_area_analysis.json` + `docs/fish_area/*.html`（2026-07-27） | **旬ピークの母数下限と定義併記**。**背景**: AdSense 診断（2026-07-27）で C4 コンテンツ品質 NG。指摘は「『海況と釣期の傾向』が釣期判断を約束しているのに単一データポイントしかない」。実データを当たると更に問題があり、`_peaks()` に母数下限が無く **278便中10便（3.6%）の旬**が「釣果が伸びやすい時期」として採用されていた（フグ×鹿島港: 分析セクション「10月下旬・10月上旬」 vs 同ページ FAQ「4/5/6月がピーク」）。両者は別定義（前者=旬別 avg_cnt、後者=月別報告件数）だが**どちらも定義を書いておらず**、同一ページ内の矛盾に見えていた。**対策**: 蒸留側 `_peaks_detailed()` に母数下限 `max(12便, 総便数の5%)` を導入し `{label,n,avg_cnt}` で母数付き出力。`_busy_months()`（月別報告件数 上位3）を新設し**両指標を並記**、表示側で「この2つは別の指標です」と定義差を明示。`_wx_for_decade()` がピーク旬の水温・波高の実勢レンジを `weather/*.csv` の実測から算出（2年以上の観測がある旬のみ・単年は誇大なので出さない）。精度の主張は `_FA_ACCURACY_WMAPE_MAX=50%` 以下のコンボのみ（wMAPE 82.8% で「上回る精度」と書いていた）。検証: (a) 蒸留 JSON の全ピークが母数下限以上かつ dict 形式（旧スキーマは fail）(b) 旬ピーク描画ページに「この2つは別の指標です」と母数「N便・平均M匹」が必ずある（docs 未再生成時は skip）。反映は日次 crawl 経由。決定ログ「2026-07-27」参照 |
| 58 | `normalize/field_reports.json` + `docs/fish_area/*.html`（2026-07-27） | **一次体験レポートの出典**。**背景**: AdSense 診断で C1 Experience / C2 Expertise が NG（「著者自身の実釣体験・テストフィッシング・現地調査に基づく記述がない」）。自動収集データをどれだけ磨いてもこの項目は動かないため、運営者本人の一次記録を載せる器を新設。`crawler.py _build_field_report_section` が fish_area の分析セクション直後に『運営者が実際に行った記録』を描画し、種別バッジ（実釣/取材/現地調査）・執筆者・実施日を必ず表示して自動集計と区別する。**出典なき体験談は「体験を装った文章」**になるため、`author`/`date`/`html` のいずれかが欠けるエントリは描画しない（レンダラ側で skip）。検証: (a) JSON 全エントリに執筆者・実施日・本文がある（0件なら「器のみ」で OK＝Experience 未充足の明示）(b) `class="field-report"` を含む docs には「自動集計データではなく」の一次情報明示がある。書き方は `normalize/FIELD_REPORTS_GUIDE.md`。決定ログ「2026-07-27」参照 |
| 59 | `crawler.py build_fish_area_pages` 孤児処理 + `docs/**/*.html`（2026-07-31） | **公開 HTML の大量削除ガード（404 フラッピング防止）**。**背景**: GSC を直接確認したところ未登録 922 件のうち **404 が 287 件**あり、その主因は fish_area の削除→再追加の往復だった（2,141本中 **573本(26.8%)** が復活を経験・2026-05 だけで 1,268本削除）。URL が 404 と 200 を往復すると Google はその URL 空間のクロール優先度を落とすため、**実在・indexable・sitemap 掲載済みの 118 ページが「検出 - インデックス未登録」のまま一度もクロールされない**状態を招いていた（`前回のクロール: 該当なし`）。**対策**: (a) 孤児 (fish, area) は `os.remove` をやめ `<meta name="robots" content="noindex, follow">` を注入する**墓標方式**に変更。200 を維持するので 404 が増えず、`build_sitemap` がディスク上の noindex を読んで自動除外し、内部リンクも生き残るので #44 を壊さない。(b) crawler 側に「対象が全体の 5%（最低 30本）を超えたらパージ自体を中止」するガードを追加。`hist_rows` の部分ロード（data/V2 CSV の truncate・CI 途中失敗）で大量削除が走る事故を防ぐ。**旧フォーマット HTML の削除のみ維持**（#19 OGP / #20 share-bar を満たせず墓標化できないため）。検証: `git diff --diff-filter=D HEAD -- docs` の HTML 削除数が、各ディレクトリ現存数の 5%（最低 30本）以内であること。意図的な一括削除時は `--allow-mass-deletion`。決定ログ「2026-07-31」参照 |
| 60 | `weather/YYYY-MM.csv`（2026-08-01） | **海況CSV 日次追記の無言停止検知**。**背景**: 2026-04-04/08 の大掃除で旧 `weather_fetch.py`（日次追記）が「不要」と誤判定されて削除され、`weather/*.csv` が 2026-04-13 で無言停止。消費側3系統（crawler.py 海況表示 / `x_post/insights.py` 平年値比較（#56 の wx_baseline）/ `crawl/build_fish_area_analysis.py` 海況実勢（#57））は生きていたため **4か月間古いCSVを読み続けた**。既存の不変条件はどれも weather/ を見ておらず検知できなかった。**対策**: (a) 日次追記を `ocean/update_weather_csv.py` として復活（crawl.yml で crawler.py の直前・continue-on-error）。形式は `export_weather_csv.py` と同一（1座標=1行・代表ポイント名写像・JST 3時間粒度）、全座標フェッチ完了後に一括追記＝「日はあるが座標が欠けたCSV」を作らない、成功座標9割未満なら追記せず非0終了。代表名は `normalize/weather_point_map.json`（座標→名前のピン留め・191座標）を最優先＝data/V2 出現回数「選挙」のドリフトで同一座標の履歴が別名に分断される事故（浦安沖→江戸川店前）の防止。(b) 検証: 最新月CSVの最大日付が today-7 以内（Open-Meteo 障害等の数日は許容）。鮮度チェックなので `--static-only` では warn（`fail_staleness`）。初期構築・欠落月の一括復旧はローカルで `rebuild_weather_cache.py --update` → `export_weather_csv.py`（weather_cache.sqlite は gitignore のため CI 不可）。決定ログ「2026-08-01」参照 |
| 61 | `normalize/area_description.json`（2026-08-01） | **エリア説明文の品質**。AI生成文（113港）に (a) 尻切れ7件（「複数の釣り物が選べる」で文が終わる） (b) 季節語と括弧内の月の矛盾19件（「秋から冬（7月・8月・12月）」＝天津港） (c) 修復時の重ね置換による「夏と夏と冬」型重複連結、が存在し一括修復。月リストはデータ由来（正）・季節語は月から機械導出。検証: 尻切れ・季節×月矛盾（**過半**・冬跨ぎ循環判定。線形判定だと81件に誤膨張）・重複連結を fail。1か月だけのズレは自然表現として許容。決定ログ「2026-08-01」参照 |
...
| 65 | `docs/area/*.html` の title/description（2026-08-19） | **登録した別称が SERP に露出していない**。`normalize/area_seo_alias.json` は「GSC で実際に表示が出た呼称ゆれ」だけを載せる SEO 資産だが、注入側に2つの取りこぼしがあった: (a) title への注入が `aliases[0]` の**1件固定**で、別称2件のうち片方が落ちる（日立久慈港「久慈漁港」＝順位5.3 で 0クリック） (b) 当日釣果2件未満で **thin テンプレに分岐すると別称の注入経路ごと外れる**（天津港は title にも description にも別称が1文字も無く、thin 23本 = CTR 0.35% 対 full 83本 2.31% ＝ 同順位帯で 6.6倍差）。どちらも**ページ自体は正常に生成される**ので既存の不変条件では全 PASS し、GSC を手集計するまで3か月気付けなかった。**契約**: title = `aliases[:2]` ＋県名（3件目以降は title が長くなるので入れない）/ description = **全件**（「〜とも呼ばれます」）。full・thin 双方で成立すること。**検証**: alias 登録エリアごとに title 先頭一致で該当ページを特定し、title に先頭2件・description に全件が含まれるか確認。別称を登録したのに area ページが見つからない場合も fail（エリア名の改称・統合でリンクが切れたことの検知）。negative test: 天津港に未露出の別称を1件足すと title/description の2件が fail することを確認。決定ログ「2026-08-06」「2026-08-19」参照 |
| 66 | `docs/sitemap.xml` の lastmod（2026-08-19） | **lastmod がビルド日一色で、クロール優先度のヒントが死んでいた**。旧実装は全 URL に lastmod=ビルド日を毎日書いており、2026/08/01 の「正直化」でも対象は x_post 日次・monthly 月報だけ。fish/area/fish_area/ship の **651本（sitemap 768本の85%）が横並びで今日を主張**していた。Google は lastmod が実態と合わないサイトの lastmod を無視するため、651本が今日と言う状態はヒント 0 と同じ。発見の経緯は GSC 手動投入中に `kotsubo` / `yokohama-shinyamashita` が「URL が Google に認識されていません」（参照元サイトマップ検出されず）だったこと。**対策**: `crawler.py _build_sitemap_lastmod_index()` で data/V2 から entity 別の最終 date を集計し lastmod に入れる（fish=tsuri_mono / area=area / fish_area=tsuri_mono×area / ship=ship）。slug が引けないページは now にフォールバック（嘘の古い日付より安全）、未来日は書かない、ハブは now のまま。実測 85%→26%。**検証**: (a) 未来日ゼロ (b) fish/area/fish_area/ship のビルド日率が 80% 以下。**マーカー方式**: 実装当日は配信中の sitemap が旧形式で必ず赤くなるため、`<!-- lastmod-source: data -->` が無い sitemap は「新コードで未再生成」と判定して warn で素通しする（#54/#62 と同じ再生成前 vacuous pass。閾値の緩和ではなく、付いた瞬間から全てブロッキング）。negative test: 旧 sitemap で 643/643(100%) を fail 検知。positive test: 新生成で 197/762(26%) を OK。決定ログ「2026-08-19」参照 |
| 67 | `docs/ship/index.html` と gnav の船宿導線（2026-08-29） | **船宿ハブが存在せず、ship ページがサイト内リンクグラフから半ば孤立していた**。`area/index.html`・`fish/index.html`・`x_post/index.html` は在るのに `ship/index.html` だけ無く、indexable 163本への導線が個別 area / fish_area からの流入のみ（内部リンク元 庄治郎丸 55本 対 金谷 137本）。さらに ship ページのヘッダは他ページと別実装の**旧5項目 gnav** で固定されており、`/x_post/` `/monthly/` `/column/` `/fish-value/` へのリンクを持たなかった（双方向で孤立）。結果、GSC 2026-08 実績は ship 全体で **768imp / 1click（CTR 0.13%）**・全ページ pos 8〜15 = page2 圏。「庄治郎丸 釣果」249imp / 0click のように**指名検索を丸ごと取りこぼしていた**（area は 3,888imp / 89click = 2.3%）。ページ自体は 66KB で薄くなく既存の不変条件では全 PASS するため、GSC を手集計するまで気付けなかった（#65 と同型の無言デグレ）。**対策**: `build_ship_index_html()` で港・エリア別の船宿ハブを生成（掲載条件は sitemap と同一 = exclude / fishing_v_zero / noindex 除外）、gnav に「船宿」を追加、ship ページのヘッダ/ボトムナビを `_v2_header_nav('ship')` / `_v2_bottom_nav('ship')` に統一、sitemap に `/ship/` を追加。**検証**: (a) ハブ存在 (b) 船宿リンク 100本以上 (c) canonical `/ship/` (d) `index.html` / `fish/index.html` / `area/index.html` / ship 個別ページの4種で gnav に `/ship/` があること。決定ログ「2026-08-29」参照 |
| 68 | `docs/area/*.html` の H1（2026-08-29） | **Google が title を捨てて H1 を SERP タイトルに使っていた**。Yahoo!検索（Google供給）の実査 n=6 で、title 維持だった金谷 5.24% / 静浦 5.01% に対し、H1 に差し替えられた飯岡 2.03% / 勝浦 1.11% / 天津 0.49%（順位帯はどれも 6.8〜8.2）。差し替え後の文字列は `<h1>{area}の船釣り釣果</h1>` と完全一致する。**帰結**: 差し替えが起きるページでは不変条件 #65（title に別称先頭2件＋県名）の効果がまるごと消える。実例が「勝浦湾 釣果」で **3位・87impr・0クリック**（勝浦湾は alias 3件目で `aliases[:2]` にも入らず、title にも H1 にも無かった）。**対策**: `area_h1_text()` を新設し H1 を `{エリア}（{別称全件}・{県名}）の船釣り釣果[【毎日更新】]` に。上限30文字、優先順位は エリア名 > 別称（全件）> 県名 > 【毎日更新】。別称を全件入れるのは title（2件上限）と役割を分けるため。【毎日更新】を先に落とすのは、差し替えが起きるページは SERP 日付が古く（勝浦11日前・天津6日前）、古い日付の隣の【毎日更新】は訴求が反転するから（2026-08-01 の【M/D更新】撤回と同じ構造）。**検証**: (a) 30文字以下 (b) 県名（ah-sub 先頭要素）が入っている (c) 素の「{エリア}の船釣り釣果」に戻っていない (d) 別称登録エリアは全件。落ちてよいのは上限に当たった場合のみで、**落ちた別称を1件戻すと必ず30を超える**ことまで確認する。negative test: 素のH1に戻す→3件 fail / 勝浦湾だけ落とす→「25文字で収まるのに落ちている」で fail / 県名を落とす→fail。決定ログ「2026-08-29」参照 |
| 69 | `docs/fish/*.html` の FAQ Q3（2026-08-30） | **title は勝っているのにスニペットが意図とズレていた**。SERP 実査で「カツオ 釣果」の title は「カツオ釣果 関東【今週42便・最高8匹】」とフル維持（競合の【2026年8月】より具体的）だったが、スニペットは FAQ Q3 から抜かれ「関東カツオ船釣りの一日の釣果は**1〜5匹が標準的なレンジ**です。**中央値は3匹**…」＝分布の説明で始まっていた。「釣果」で調べる人が見たいのは直近に何が釣れたかで、意図とズレるうえ数が小さく見える（GSC 2026-08: 120impr / **0click**）。HERO も title も「今週42便・0〜8匹」と言っているのに SERP に出る文章だけが統計だった。**対策**: `_build_fish_count_q3_text()` に `recent_lead` を追加し Q3 の先頭に「直近7日は{N}便の報告があり、個人釣果は{レンジ}でした。」を置く。数値は HERO・title と同じ `len(catches)` / `cnt_range_str` を使う（別集計で食い違うと同一ページに数字が2種類出る）。HERO にレンジが出ない24魚種は対象外（無いものを書かない）。統計部分は消さず先頭を入れ替えただけで情報は減らない。補遺3 遵守。**検証**: (a) 対象魚種の Q3 が「直近7日は…」で始まる (b) **その数値が HERO と一致する**（食い違い検知が本体）。negative test: リード除去 → fail / 便数を 42→99 に改竄 → 「HERO と食い違う」で fail。⚠ Google は抜粋元を指定させないので効くかは未証明。判定は `analytics/serp/` で「カツオ 釣果」を再実査（2026-09-12 以降）。決定ログ「2026-08-30」参照 |
| 70 | `docs/area/*.html` thin テンプレの H1（2026-09-03） | **#68 の副作用で、当日釣果が無いページの H1 だけが【毎日更新】を名乗っていた**。thin テンプレ（当日釣果なし・20本）の title は 2026-08-06 の判断で `{エリア}（別称・県名）の船釣り釣果・過去実績` にしてあり、body も「本日の釣果報告は集計待ち」。ところが #68 で thin パスにも `area_h1_text()` を通した際、【毎日更新】が無条件で付いた。#68 の前提そのもの（**Google が title を捨てて H1 を SERP タイトルに使う**）が成り立つと、SERP で「毎日更新」と読ませてクリックさせた先が「集計待ち」になる。2026-08-06 の判断（嘘はつかない・ただし最初に見せる情報ではない）と正面から矛盾する。GSC 2026-07〜08 実績（**ページ次元＝真値**）で thin 20本は 975impr / 21click（2.15%）、full 134本は 14,636impr / 346click（2.36%）＝**ほぼ同じ**。⚠ 当初はクエリ次元 CSV で「thin 0.67% / full 2.28%」と見えていたが、これは GSC の匿名化欠測（thin ほど低頻度クエリ比率が高く落ちやすい）による幻だった。2026-08-06 に thin の title/desc を作り直した際の根拠「thin 0.35% / full 2.31%」も同じバイアスを受けている。**対策**: `area_h1_text(..., daily=False)` を追加し、thin パスの H1 末尾を 【毎日更新】ではなく「・過去実績」にして title と揃える（30文字上限・優先順位は #68 のまま）。**検証**: title に「・過去実績 |」を含むページの H1 に【毎日更新】が無いこと。negative test: `daily=False` を外す → 20本 fail。⚠ **CTR 改善の施策ではない**（thin の CTR は元から full 並み）。目的は「書いてあることを本当にする」ことで、AdSense 4連敗の理由（「有用性の低いコンテンツ」）に効くかは未証明。決定ログ「2026-09-03」参照 |
| 71 | `docs/fish/index.html` の網羅（2026-09-03） | **ハブが 73本中3本にリンクしていなかった**。魚種一覧は ①`_FISH_ROMAJI` 登録済み ②fish_area ページが1件以上ある の**両方**を満たす魚種しか載せない。両方から漏れると、ページは実在し sitemap にも載るのに内部リンクが1本も無い状態になる。実例: `sujiika.html`（romaji 登録済みだが fish_area ページ無し）、`アカアマダイ.html` / `ハゼ.html`（`_FISH_ROMAJI` 未登録で URL が日本語のまま）。**対策**: `_fish_index_orphan_sweep()` を最終保険として追加し、`docs/fish/*.html` のうちハブから未リンクのものを「その他の魚種」として末尾に足す（0本なら何も足さない）。`_sweep_dead_internal_links()` と同じ「最後に現物を見て直す」思想。**検証**: `docs/fish/*.html` 全件が `href="{fn}"` か URL エンコード形で index.html に出現すること。negative test: 追加ブロックを削る → 3件 fail。⚠ 2026-09-03 の URL Inspection 実査では fish の未インデックスの真因は**クロール到達性**（fugu は被リンク663本でも未クロール）であり、内部リンクの穴は真因ではない。ただし塞げる穴を開けておく理由が無いので塞ぐ。決定ログ「2026-09-03（続2）」参照 |
