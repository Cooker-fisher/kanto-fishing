...
| 70 | `docs/area/*.html` thin テンプレの H1（2026-09-05） | **#68 の副作用で、当日釣果が無いページの H1 だけが【毎日更新】を名乗っていた**。thin テンプレ（当日釣果なし・20本）の title は 2026-08-06 の判断で `{エリア}（別称・県名）の船釣り釣果・過去実績` にしてあり、body も「本日の釣果報告は集計待ち」。ところが #68 で thin パスにも `area_h1_text()` を通した際、【毎日更新】が無条件で付いた。#68 の前提そのもの（**Google が title を捨てて H1 を SERP タイトルに使う**）が成り立つと、SERP で「毎日更新」と読ませてクリックさせた先が「集計待ち」になる。2026-08-06 の判断（嘘はつかない・ただし最初に見せる情報ではない）と正面から矛盾する。GSC 2026-07〜08 実績（**ページ次元＝真値**）で thin 20本は 975impr / 21click（2.15%）、full 134本は 14,636impr / 346click（2.36%）＝**ほぼ同じ**。⚠ 当初はクエリ次元 CSV で「thin 0.67% / full 2.28%」と見えていたが、これは GSC の匿名化欠測（thin ほど低頻度クエリ比率が高く落ちやすい）による幻だった。2026-08-06 に thin の title/desc を作り直した際の根拠「thin 0.35% / full 2.31%」も同じバイアスを受けている。**対策**: `area_h1_text(..., daily=False)` を追加し、thin パスの H1 末尾を 【毎日更新】ではなく「・過去実績」にして title と揃える（30文字上限・優先順位は #68 のまま）。**検証**: title に「・過去実績 |」を含むページの H1 に【毎日更新】が無いこと。negative test: `daily=False` を外す → 20本 fail。⚠ **CTR 改善の施策ではない**（thin の CTR は元から full 並み）。目的は「書いてあることを本当にする」ことで、AdSense 4連敗の理由（「有用性の低いコンテンツ」）に効くかは未証明。決定ログ「2026-09-05」参照 |
| 71 | `docs/fish/index.html` の網羅（2026-09-05） | **ハブが 73本中3本にリンクしていなかった**。魚種一覧は ①`_FISH_ROMAJI` 登録済み ②fish_area ページが1件以上ある の**両方**を満たす魚種しか載せない。両方から漏れると、ページは実在し sitemap にも載るのに内部リンクが1本も無い状態になる。実例: `sujiika.html`（romaji 登録済みだが fish_area ページ無し）、`アカアマダイ.html` / `ハゼ.html`（`_FISH_ROMAJI` 未登録で URL が日本語のまま）。**対策**: `_fish_index_orphan_sweep()` を最終保険として追加し、`docs/fish/*.html` のうちハブから未リンクのものを「その他の魚種」として末尾に足す（0本なら何も足さない）。`_sweep_dead_internal_links()` と同じ「最後に現物を見て直す」思想。**検証**: `docs/fish/*.html` 全件が `href="{fn}"` か URL エンコード形で index.html に出現すること。negative test: 追加ブロックを削る → 3件 fail。⚠ 2026-09-05 の URL Inspection 実査では fish の未インデックスの真因は**クロール到達性**（fugu は被リンク663本でも未クロール）であり、内部リンクの穴は真因ではない。ただし塞げる穴を開けておく理由が無いので塞ぐ。決定ログ「2026-09-05（続2）」参照 |
