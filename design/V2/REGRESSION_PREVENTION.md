...
| 51 | `docs/fish-value/`＋`docs/index.html`＋`docs/sitemap.xml`（2026-07-05） | 釣果価値チェッカーのリリース整合。(1) `fish-value/index.html` に noindex が**無い**（リリース済み）(2) `docs/index.html` に `/fish-value/` 導線あり（gnav＋トップカード）(3) `sitemap.xml` に `/fish-value/` 収録 (4) 価格マスタ鮮度: `fish-price-master.json` の `seasonal.data_month` ラグが3か月超で warn（`urls_manifest.json` への新月報追記漏れ検知・月報公開ラグにより正常時1.5〜2.5か月）。fish-value は crawler.py 非生成の独立アプリのため、導線・sitemap は crawler.py 側（gnav/カード/build_sitemap）が SoT。決定ログ「2026-07-05」参照 |
