...
| 66 | `docs/area/*.html` thin テンプレ本文（2026-08-17） | **thin ページの本文が否定文2連発で始まっていた**。GSC 実測で同順位帯なのに thin 0.78%（515表示/4クリック・pos8.56）vs full 2.28%（3,425表示/78クリック・pos8.03）＝ **2.9倍**の CTR 差。2026-08-06 に title/description は full と揃えたが**本文は手つかず**で、決定ログ 2026-08-01 の SERP 実査どおり Google は meta description をほぼ無視して本文を抜粋するため、スニペットが「本日の釣果報告は集計待ち」→「出船報告はまだ届いていません」になっていた。さらに full にしかない **海況データ**（出船報告と独立に毎日更新される唯一のデータ＝出船が途切れている港ほど「今日は出せるのか」の判断材料になる）と **area-all-fish**（fish_area への内部リンクを張る唯一の場所）がthin だけ欠けていた。**対策**: 実績リード文を冒頭に置き「集計待ち」notice はその後ろへ（description と同じ「嘘はつかない・ただし最初に見せない」）、ヒーロー `.ah-m` を実績数字に、海況/全魚種セクションを thin にも出力。マークアップの SoT を `build_area_sea_section_html` / `build_area_all_fish_section_html` に寄せて full と共用。**検証**: thin ページ（title に「の船釣り釣果・過去実績」）について (a) 実績リードが `.notice` より前 (b) `.ah-m` が「集計待ち」でない (c) fish_area が実在するエリアなら `area-all-fish` あり (d) full の過半に海況があるなら thin にもある。**新テンプレが1本も出ていない間は vacuous pass**（[62] と同じ。crawler.py の次の日次フル実行まで validate.yml が赤くなるのを避ける）。決定ログ「2026-08-17」参照 |
...
| ファイル | 役割 |
|---|---|
| `crawl/validate_output.py` | 不変条件（#1〜#66）の検証 gatekeeper |
