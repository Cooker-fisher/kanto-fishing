...
**suion_raw / suishoku_raw は全件 null**。自社HPで構造化フィールドとして存在する場合はここに値が入る。

## 設計原則: raw json は生データの器、CSV が料理する

**catches_raw.json の15フィールドは変更しない。**

- raw json = HPから取れた生テキストをそのまま格納する器
- CSV = raw json を正規化・構造化して分析に使える形にする層
- 自社HPから取れた情報は**既存15フィールドに収める**:
  - 構造化フィールド（水温・水色等） → `suion_raw` / `suishoku_raw` に入れる
  - ナラティブ型（たいぞう丸等） → `kanso_raw` にフリーテキストとして入れる → CSVで抽出
- **CSV列の追加**（source列等）は既存データに影響しないため、ガバナンス不要で実施可能

### ソース識別（未決定）

釣りビジョン経由か自社HP直接かの見分け方は検討中。候補:
