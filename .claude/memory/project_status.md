...
**GSC 月次（2026-09-05 に真値へ訂正）**: 8月 **424click / 21,970impr**（7月 292 / 11,933）。
旧記載「8月 120click / 6,263impr」は**クエリ次元 CSV の値で、GSC の匿名化により
真値の約 1/3 しか入っていなかった**。詳細は `analytics/README.md` と決定ログ 2026-09-05。
...
**インデックス（2026-09-05 に内訳を実測して再解釈）**: docs/ の HTML **2,187本のうち
...
**fish の魚種ページ 14本が未登録**。内部リンクでも sitemap でもない（fugu は被リンク 663・
sitemap 収載済み）。

⚠ **「データ量が足りないだけ」は 2026-09-05 に棄却した。** 同日 `lastCrawlTime` を見たら
**fugu は一度もクロールされていなかった**（本文3,570字・被リンク663本・sitemap 収載でも）。
読まれていないページの本文量は原因になり得ない。sawara は 2026-04-15 から sitemap にいて
なお「URL が認識されていません」、akaika は**新設（2026-08-08）より前の 404 判定**が残存。
**律速はクロール到達性**で、本文増強では解けない。投入順は
`analytics/gsc/manual_index_requests.md` に実データ根拠つきで固定した。

なお `docs/fish/index.html` は **70/73 本しかリンクしていない**（`sujiika.html` は
romaji slug があるのに欠落・`アカアマダイ.html` / `ハゼ.html` は `fish_romaji_map.json`
未登録で URL が日本語のまま）。手動投入の枠を使う前にハブ側を直すほうが先。
...
**判定はまだできない**（2026-09-05 時点で GSC は 09-04 まで・08-31 週は 5/7日）。
...
- **残**: 港・船宿の体験（fish_area 近重複135本に最も効く・未着手）／C2 の
  fish_area_notes 拡大
- **2026-09-05: 19種 → 23種**。未インデックスだった サワラ・イトヨリ・マハタ・アカイカ に
...
4. ~~fish_area 薄ページの物理削除/統合~~ → **2026-09-05 に一部着手**。
