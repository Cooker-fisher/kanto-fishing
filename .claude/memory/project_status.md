...
## 🚨 2026-08-31 サイト全体の順位下落（最優先・原因未確定）

**8月末に流入が半減し、9月に入っても戻っていない。** 施策の効果判定より先にこれ。

| | 08-22〜08-30(9日) | 09-03〜09-11(9日) |
|---|---:|---:|
| impr 合計 | 6,296 | **2,213**（-65%） |
| imp 加重 pos | 8.7 | **12.3** |
| 露出のあった URL 数 | 362 | 219 |

セクション別: area -54% / fish -65% / ship -81% / fish_area -83% / ルート直下 -98%。
**トップページだけ +29%。** 指名検索は無事で、それ以外が全部落ちている。

### 分かっていること（実測）

- **索引の問題ではない。** URL Inspection 12本（下落組8＋対照4）が全て
  「送信して登録されました」/ fetch SUCCESSFUL / robots ALLOWED / canonical 自己
- **当方の変更では説明できない。** `docs/calendar.html` は 08-30 と 09-11 で
  生成日スタンプ以外**バイト一致**なのに 506impr pos8.2 → 8impr pos49.8。
  sitemap 収載数（793〜818）も robots.txt のハッシュも不変
- **需要減でもない。** 「9月 船釣り 関東」が 8月 17impr pos8.2 → 9月 **0impr**。
  9月に需要が増えるはずのクエリで消えている
- 断点は **08-30 511impr → 08-31 286impr** の1日。以後 200〜290 で定着し、
  pos は 09-06 の 15.6 を底に 11.8 まで部分回復

### 分かっていないこと

- **原因。** Google の August 2026 spam update は 08-18〜08-21 で確定だが、
  当サイトの断点は 08-31 で10日ずれる。その後も未告知の変動が続いたと
  複数のトラッカーが報告しているが、**因果は未証明**
- 手動対策（ペナルティ）の有無は GSC 画面でまだ確認していない ← **次にやること**

### これで死んだもの

**#67 / #68 / #69 / #70（すべて 2026-08-29 投入）の効果判定は不能。**
投入2日後に全体が動いたので、上がっても下がっても施策に帰属できない。
`analytics/gsc/trend.py` の判定基準に ⛔ を明記した。

### 再発検知

`trend.py` に `detect_break()` を入れた。日次サイト合計を前後7日で比べ、
30%以上の階段状の落ち込みをレポート冒頭で警告する。今回の 08-31 は
独立に検出できることを確認済み（4,702 → 1,807 / pos 8.7 → 12.3）。
**今回はたまたま週次表を眺めていて気づいた。次は自動で気づく。**

---

## SEO 現在地（2026-08 実測・⚠ 上の全体下落より前の数字）
