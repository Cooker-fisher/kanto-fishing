...
### 1. 集計単位を「魚種」→「釣り物」に変更（中期課題）

**問題:**
現在 `history.json` / `calc_combo_scores()` は「その魚種が釣れた記録」を全件集計している。
アジ便の外道マダイ（数匹）とマダイ便の本命マダイ（数十匹）が混在し、
マダイの平均釣果・スコアが歪む。予測精度にも悪影響。

**方針:**
`trip_type`（v5.18 で取得開始）を使い、集計時に本命のみ抽出する。
```
is_honmei = fish in (c.get("trip_type") or "")
```
外道レコードはスコアリング・history 集計から除外し、別途「外道期待値」として保持する。

**前提条件:**
- `trip_type` の実機確認が完了してから実装（section 0 を先に済ませること）
- `trip_type` の表記ゆれ対応（「LT五目」「ライトアジ」→ アジ、など正規化が必要）
