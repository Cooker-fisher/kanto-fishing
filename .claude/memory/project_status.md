...
## ✅ 日報（x_post）本文刷新（2026/07/22・main agent）— 推測フィラー全廃・数値根拠のみ

**詳細: 90_決定ログ「2026-07-22」／不変条件 #56。ユーザー指摘「ハイライトも魚種別報告も薄っぺらい」起点**

- 原因: ハイライト＝H文型1本＋カード2枚、魚種別散文＝`_fish_map` の**10魚種ハードコード**のみ。
  文面は「好海況が観測されています」「組み込みやすい魚種です」等の**根拠なしフィラー**。
  比較軸（平年比・船宿別内訳・記録性）が皆無で、`season_ratio_*` は analysis.sqlite 依存＝**CI で常に1.0**だった。
- 実装: **`x_post/insights.py`**（data/V2 CSV のみで平年比＝過去3年の同旬 cnt_max 中央値比／上位3便／
  N日ぶり／直近60日順位／型平年比。当日より前だけ集計＝二重計上防止・0.7秒/日）＋
  **`x_post/narrative.py`**（カード最大4枚・ハイライト3〜5文・海況は実測値のみ・魚種別は2便以上全魚種＋単発まとめ）。
  テーブルに平年比 chip（母数8便未満は非表示・±10%以内は「平年並み」）。翌日予測は #53 と同じ tier A 関門経由。
- 効果（7/21 実測）: 散文 7魚種→**26魚種**、ハイライト 1文→5文（全て数値付き）、カード 2→4枚。
- **反映は次回の日次 crawl**（過去ページは遡及再生成しない＝CSV由来だと船宿数/魚種数が変わるため）。
  #56 は `class="commentary evidence"` を持つページのみ検査するので旧ページは skip される。
- 残: `templates.py` H/F 文型はフォールバック用に残置（掃除は別タスク）／海況リードの簡易平年表は未精緻化。
...
## ✅ T47b 完了（2026/07/17）— 表示予測を検証済みモデルへ差し替え（選別公開の完成）

**詳細: 90_決定ログ「2026/07/17 T47b」。mockup: design/V2/mockup-forecast-verified.html**

- forecast 日付/週次ページの予測カードを「前日CIコミットの forecast_daily.json × open_tier tier A」に差し替え。
  **「サイトに出る予測」と「測定してきた精度」が初めて一致**（全レンジがボウズ込み的中率90%以上の実測付き）
- 旧ペイウォール（月額500円/blur）完全撤去＝#52 Phase2 完了。過去日付ページ95枚を掃引（毎ビルド自動）
- 週次は weekly_ok（H=14/21/28 でも基準クリア）106組のみ+傾向注記（domain指摘: H=0の無検証外挿禁止）
- 不変条件 **#53** 新設（tier整合/ペイウォール残留/鮮度）。validate errors=0/warnings=0
- 内蔵モデル（predict_catches）は海況・エリアチップ・hub 概況用に降格存続
- 残: 死にコード build_forecast_section 掃除 / index「今週末の狙い目」の検証済み化検討 /
  「先週の予測 vs 実績」ページ（predict_log 起点・信頼構築）
