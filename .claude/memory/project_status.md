...
### 1. 「釣り物」軸の導入（大規模・生データ運用軌道後に着手）

**背景:**
現在の設計は「魚種」軸（釣れた魚）でページ生成・集計・予測がすべて構築されている。
しかし「釣り物（ターゲット）」と「魚種（釣れた魚）」は別物で、外道が混入すると数値が歪む。
例: マダイ便で外道アジが数匹 → アジの平均釣果を下げる / アジ便の外道マダイ → マダイの予測を歪める

**影響範囲（大きい）:**
- `history.json` の集計ロジック全体
- `calc_combo_scores()` のスコアリング
- `build_fish_area_pages()` / `build_fish_pages()` のページ構造
- 予測エンジン（predict_catches）

**本命/外道の判定方針:**
`trip_no` の順番で判定できる可能性が高い。
- `trip_no = 1` → 本命（船宿が最初に書く = メインターゲット）
- `trip_no = 2, 3, ...` → 外道（嬉しい外道 or ついで）

ただし近縁種（例: ヤリイカ/スルメイカ）は船宿によって書き順が異なる場合がある。
→ **船宿ごとに書き方の傾向を確認する必要がある**（生データ蓄積後に検証）

**設計方針:**
`trip_type`（v5.18 で取得開始）に釣り物データが溜まってから、
「魚種」軸と「釣り物」軸を並存させる設計に移行する。
生データ（catches_all.json）の蓄積が進んだタイミングで設計し直す。

**前提条件:**
- `trip_type` 実機確認 + 表記ゆれ正規化が完了していること
- catches_all.json に十分な生データが蓄積されていること（目安: 数週間分）
- 船宿ごとの書き方パターンを把握していること
