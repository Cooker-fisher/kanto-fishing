...
MAX_FACTORS        = 12   # 全体上限
MAX_CMEMS_DEFAULT  = 0    # 全魚種: CMEMS除外（偽相関過学習防止・2026/04/18確定）
# MAX_CMEMS=0 の根拠:
#   - 全種実験でBL2勝率 84.4% → 88.5%（+4.1pt）に改善
#   - 回遊魚・深海魚（CMEMS有望候補）でも悪化（イナダ +5.5pt、クロムツ BL2 100%→0%）
#   - CMEMS変数はn小コンボで偽相関を掴む → 適応的閾値でも防ぎきれない
#   - CMEMSは将来: エリア別月次集計の obs_factor として再検討
