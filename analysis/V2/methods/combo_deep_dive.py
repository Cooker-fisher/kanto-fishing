...
_FAST_MAX_H_OVERRIDE: dict = {
    ...
    # ※メバル×幸栄丸(FAST_MAX_H=1試行): H=3は改善したがH=7が+10.2→+14.9pt悪化。FAST因子がH=7でも有効なため除外しない
    # ※クロダイ×平作丸(FAST_MAX_H=3試行): H=7が+0.2→+4.2pt悪化。FAST因子がH=7でも有効。デフォルト維持
}
...
FACTOR_BLACKLIST: dict[tuple, set] = {
    ...
    # 冬季専用コンボ × 台風擬似相関: 台風シーズン外(11〜3月)釣期と台風接近期のデータ分離で高r発生
