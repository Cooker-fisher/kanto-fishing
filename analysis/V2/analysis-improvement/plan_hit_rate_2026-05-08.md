...
### ⚠ 出力形式の絶対制約（2026/05/08 補遺2 + 補遺3 で確定）

**出力は「単一レンジ」の min/max 2値のみ。avg は出さない。重ねレンジ禁止。**

| 要素 | 出力形式 | 禁止例 |
|---|---|---|
| 数 | `min匹 〜 max匹`（2値・レンジ1つ）| `min: A匹〜B匹` のように pred_lo/pred_hi 各々に区間予測をつけない |
| 型 | `min cm 〜 max cm`（2値・レンジ1つ）| 同上 |
| 重 | `min kg 〜 max kg`（2値・レンジ1つ）| 同上 |

**avg を出さない理由（補遺3 確定）:** 釣果報告における min/max は「同船者の幅」であり、avg は単純平均でしかなく統計的意味を持たない（"1,1,1,100" と "1,99,99,99,100" は分布が異なるが avg はほぼ同じ）。
...
- 内部評価用: avg は learning/評価では引き続き利用（combo_decadal の baseline 等）。**ユーザーへの表示には出さない**
...
5. **avg/ave を表示する**（補遺3 で追加）

参照: CLAUDE.md マネタイズ方針セクション・90_決定ログ.md「2026/05/08 補遺3」
