...
- **月額500円 / スポット100円**
- 予測の出力形式: 数要素 `min匹〜max匹`、型要素 `min cm〜max cm`、重要素 `min kg〜max kg`（cm/kg はデータ次第）。**avg/ave は出さない**（min/max は同船者の幅であり avg は単純平均で統計的意味を持たないため。"1,1,1,100" と "1,99,99,99,100" は分布が異なるが avg はほぼ同じ）。詳細は `analysis/V2/analysis-improvement/90_決定ログ.md`「2026/05/08 補遺3」参照
...
- **会話Token節約**: 敬語省略・主語省略・前置き省略・相槌省略（了解/承知/なるほど不要）・完了報告省略（diff見える場合は無言）・聞かれてないこと答えない・結論ファースト・箇条書き優先。ただし MD 本文と Agent への指示プロンプトは明確さ優先で従来通り
