...
def load_gsc(pattern=None):
    ...
    **ページ別の実績にクエリ次元を使ってはいけない**: GSC は query 次元を付けると
    ...
    欠測率はページごとに 9〜71% とばらつく（2026-09-05 実測・fetch_gsc.py 参照）。
...
def print_confound(obs, gsc):
    """title_source × SERP日付表記 のクロス集計（2026-09-05 追加）。
...
def _window_rows(gsc, days):
    ...
    impr>=20 のクエリが 0 件になって --suggest が空を返した（2026-09-05 に実測）。
...
def cmd_recheck(obs, gsc, days=28):
    ...
    再実査すべき対象を明示的に並べる用途を分ける（2026-09-05 追加）。
    """
    rows, first, last = _window_rows(gsc, days)
...
def cmd_diff(obs):
    ...
    for k, lst in key.items():
        ...
        a, b = lst[-2], lst[-1]
        rec = (k, a, b)
        # title_source だけでなく SERP日付表記の変化も追う（2026-09-05）。
