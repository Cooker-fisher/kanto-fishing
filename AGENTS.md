...
**コンセプト**: 無料=当日釣果（客集め）／有料=1週間日別+2/3/4週後予想（収益）
...
|------|------|
| ホスティング | GitHub Pages |
| 独自ドメイン | funatsuri-yoso.com |
| DNS | お名前.com（01〜04.dnsv.jp）→ A レコード GitHub Pages IP |
| HTTPS | Enforce HTTPS |
| 自動更新 | GitHub Actions（毎日16:30 JST / cron `30 7 * * *`） |
| 手動実行 | GitHub Actions Run workflow（workflow_dispatch） |
...
│       │   ├── _paths.py       # パス自動解決（AGENTS.md を目印にルート検出）
...
- **月額500円 / スポット100円**
- 予測出力: 数=`min匹〜max匹` / 型=`min cm〜max cm` / 重=`min kg〜max kg`。**avg は出さない**（"1,1,1,100" と "1,99,99,99,100" の avg がほぼ同じだから）。詳細 `90_決定ログ.md`「補遺3」
- 現状MAPE 27〜55%（CV低い船宿）→ 船宿×魚種・エリア船宿×魚種単位で改善
- 決済・UI 未決定
...
### 釣果収集・表示（A/B/E）
1. ✅ 釣果自動収集（釣りビジョン・毎日）
2. ✅ 魚種カードタップでランキング（船宿TOP10・バーグラフ）
3. ✅ 今週末の狙い目（★評価+理由タグ+100パターン）
4. ✅ 魚種別ページ（SEO対応）
5. ✅ エリア別ページ・カレンダーページ
6. ✅ discover_ships.py（船宿SID自動収集 89船宿）

### データ収集・分析（A/C）
7. ✅ 気象・海況（153座標×145万行）
8. ✅ 台風トラック（70台風・2,475ポイント）
9. ✅ 月齢・潮汐（1,190日分）
10. ✅ 釣果×気象相関（combo_deep_dive.py・55種/45種完了）
11. ✅ 欠航閾値計算
12. ✅ 予測精度バックテスト（H=0,1,3,7,14,21,28）
...
- crawler.py は**標準ライブラリのみ**
- 釣りビジョンへのリクエストは 0.8秒待機（サーバー負荷対策）
- GitHub Actions のタイムゾーンは UTC（16:30 JST = 07:30 UTC）
- fish/*.html のファイル名は日本語（URLエンコードされる）
- **weather_cache.sqlite は .gitignore**（約400MB・ローカルのみ）
- **変更前に PIPELINE.md の変更インパクトマトリクスを確認**
...
- 粒度の違うデータを混ぜない
- 勝手に実装しない
- **JSONキー取得はエージェント要約信頼せず Bash で直接確認**
- **ループ実行前に `print(f'対象: {len(list)}件 先頭3: {list[:3]}')` で目視確認**
- **会話Token節約**: 敬語省略・主語省略・前置き省略・相槌省略・完了報告省略（diff見える場合）・聞かれてないこと答えない・結論ファースト・箇条書き優先。MD本文とAgent指示は対象外

---

@.Codex/memory/project_status.md
@.Codex/memory/feedback_crawler_editing.md
@.Codex/memory/feedback_regression_prevention.md
